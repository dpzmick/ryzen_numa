...
run_test( size_t core1, size_t core2 )
{
  ...
  uint64_t mb = sizeof( chunk_t )*N_CHUNKS/1024/1024;
...
bw( void ) {
  ...
  // memcpy is faster, but this is more comparable to what the rest of
  // the code is doing
...
{
  bw();
  printf( "%zu entries in data\n",  (CACHE_LINE*CHUNK_LINES)/sizeof( m256 ) );
  printf( "\nfast tests\n" );
  run_test( 0, 1 );
  run_test( 1, 2 );
  run_test( 2, 3 );
